...
.edit_form_coach_component, .slot_outer{
  display: inline-flex;
}
</style>
<template>
  <div class="edit_form_coach_component">
    <template v-if="isDelete">
      <a-popconfirm
        placement="left"
        title="Are you sure delete this Coach?"
        ok-text="Yes"
        cancel-text="No"
        @confirm="deleteForm"
        >
...
export default {
  ...
  methods: {
    deleteForm(){
      ...
      formData.append('action', 'admin_user_coach_destroy');
    ...
    storeRequest(){
      this.loading = true;
      let formData = new FormData;
      formData.append('action', 'admin_user_coach_store');
    ...
    updateRequest(){
      this.loading = true;
      let formData = new FormData;
      formData.append('action', 'admin_user_coach_update');
      formData.append('id', this.id);
      formData.append('entity', JSON.stringify(this.form));
    ...
    getEntity(){
      ...
      this.loading = true;
      let formData = new FormData;
      formData.append('action', 'admin_user_coach_show');
      formData.append('id', this.id);
  ...
  mounted() {
    if(!window.editCoachForm){
      window.editCoachForm = [];
    }
    window.editCoachForm.push(this);
  },
};
</script>
