...
<template>
  <div class="OUTER_USER_ORGANIZATIONS">
    ...
    <a-table
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'count_answers'">
          <span @click.stop='goToOrganizationAnswers(record.id)' class='answers_form'>
...
import EditOrganizationForm from 'vueadmin/Components/EditOrganizationForm.vue';
...
export default {
  ...
  methods: {
    fullUrl(slug){
      return `${location.protocol}//${location.hostname}/form-start?organization=${slug}`;
    },
    downloadXl(token){
      window.open(location.protocol+'//'+location.hostname+'/?resultExportCSV='+token);
    },
    goToOrganizationAnswers(organizationId){
      this.$router.push({path: location.pathname, query: {page: 'user-forms', organization: organizationId}});
    },
    ...
    copyLink(record){
      let link = location.protocol+'//'+location.host+'/form-start/?organization='+record.slug;
    ...
    loadApiData(params = {}) {
      ...
      form_data.append('action', 'admin_user_organizations_index');
